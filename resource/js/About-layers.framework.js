jQuery(function ($) {
  ...
  $(
    ".media-image, .thumbnail-media, .widget.slide .image-container, .post .thumbnail"
  ).fitVids();
...
function layers_swiper_resize(s) {
  ...
  s.slides.each(function (key, slide) {
    ...
    if (jQuery(slide).find(".content").outerHeight())
      slide_height += jQuery(slide).find(".content").outerHeight();
    if (jQuery(slide).find(".content").height())
      slide_height -= jQuery(slide).find(".content").height();
...
function layers_apply_overlay_header_styles() {
  ...
  if ($header.hasClass("header-overlay")) {
    ...
    if ("A" == $first_element.prop("tagName"))
      $first_element = $content_wrapper.children(".widget").eq(0);
    if (
      $first_element.hasClass("slide") &&
      !$first_element.hasClass(".full-screen")
    ) {
