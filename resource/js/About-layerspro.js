jQuery(function ($) {
  ...
  function resizeVideos() {
    $(".swiper-container .layerspro-slider-video").each(function () {
      ...
      $that.removeClass(
        "layers-slider-video-ultra-wide layers-slider-video-super-wide layers-slider-video-wide layers-slider-video-square layers-slider-video-tall layers-slider-video-super-tall"
      );
      if ($ratio <= 20) {
  ...
  $(document).on(
    "click",
    ".header-search > a, .menui-item-layers-search > a",
    function () {
  ...
  if (bodyEl.hasClass("layers-animate")) {
    ...
    $(".header-site").addClass(
      "do-animate delay-200 animated-1s layers-default-fade-in-down"
    );
    $(".header-block.container").addClass(
      "do-animate delay-200 animated-1s layers-default-fade-in-up"
    );
    $(sectionsToAnimate.join(","))
      .not(".do-animate")
      .addClass("do-animate translucent animated-1s layers-default-fade-in-up");
...
function layers_pro_init_post_carousel(widget_id, settings) {
  ...
  var mimic_row = jQuery(
    '<div class="layers-pro-post-carousel-mimic-row"><div class="grid">'
  );
  var mimic_row_grid = mimic_row.find(".grid");
  jQuery(widget_id).prepend(mimic_row);
  mimic_row.addClass(carousel_row.attr("class"));
  mimic_row_grid.prepend(
    jQuery(widget_id).find(".grid").children(":lt(6)").clone()
  );
  mimic_row_grid
    .children()
    .html("")
    .prepend('<div style="background:black">&nbsp;</div>');
  carousel_row_grid
    .children()
    .not(".swiper-pagination, .arrows")
  ...
  carousel_element
    .find(".column, .column-flush")
    .removeClass(
      "column column-flush span-1 span-2 span-3 span-4 span-5 span-6 span-7 span-8 span-9 span-10 span-11 span-12"
    )
    .addClass("swiper-slide");
  var space_between = layers_pro_post_carousel_calculate_space_between(
    carousel_row,
    mimic_row
  );
  var per_row = layers_pro_post_carousel_calculate_per_row(
    carousel_row,
    mimic_row
  );
  carousel_row.css({
    "padding-left": layers_pro_post_carousel_calculate_holder_spacing(
      carousel_row,
      mimic_row
    ),
    "padding-right": layers_pro_post_carousel_calculate_holder_spacing(
      carousel_row,
      mimic_row
    ),
  });
...
function layers_pro_post_carousel_resize(post_swiper, carousel_row, mimic_row) {
  ...
  var holder_padding = layers_pro_post_carousel_calculate_holder_spacing(
    carousel_row,
    mimic_row
  );
  carousel_row.css({
    "padding-left": holder_padding,
    "padding-right": holder_padding,
  });
...
function layers_pro_post_carousel_calculate_per_row(carousel_row, mimic_row) {
  ...
  mimic_row.show();
  var per_row = parseInt(
    Math.round(mimic_row.width() / column_element.width())
  );
  mimic_row.hide();
  return per_row;
}
function layers_pro_post_carousel_calculate_space_between(
  carousel_row,
  mimic_row
) {
  var column_element = mimic_row.find(".column, .column-flush").eq(0);
  mimic_row.show();
  var space_between =
    parseInt(column_element.css("margin-left")) +
    parseInt(column_element.css("margin-right"));
  mimic_row.hide();
  return space_between;
}
function layers_pro_post_carousel_calculate_holder_spacing(
  carousel_row,
  mimic_row
) {
  var column_element = mimic_row.find(".column, .column-flush").eq(0);
...
function layers_pro_post_carousel_match_heights(post_swiper, match_height) {
  ...
  post_swiper.container
    .find(".thumbnail-body .button")
    .css({ "margin-top": "", transition: "0s,0s" });
  ...
  post_swiper.slides.each(function (key, slide) {
    ...
    if (jQuery(slide).find(".thumbnail-body").length) {
      ...
      var $body_height =
        parseInt(slide_height) -
        parseInt(thumb_height) -
        parseInt(padding_top) -
        parseInt(padding_bottom);
...
(function ($, $window) {
  $window.layersAnimationHandler = function (
    replacementClass,
    addDefaultAnimation
  ) {
    return function () {
